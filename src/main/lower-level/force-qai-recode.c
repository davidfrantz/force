...
small *qim_ = NULL;
  ...
  QIM  = copy_brick(QAI, 1, _DT_SMALL_);
  if ((qim_ = get_band_small(QIM, 0)) == NULL){
  ...
  set_brick_save(QIM, 0, true);

  set_brick_bandname(QIM, 0, "CMIX-II cloud re-classification");           
  set_brick_nodata(QIM, 0, 255);
  ...
  #pragma omp parallel shared(qim_, QAI) default(none)
  {
    ...
    for (int p=0; p<get_brick_ncells(QAI); p++){
      ...
      // translate bits hierarchically into CMIX-II classes
      if (get_off(QAI, p)){ // off = nodata
        qim_[p] = 255;
      } else if (get_cloud(QAI, p) == 2){ // thick cloud = cloud
        qim_[p] = 1;
      } else if (get_cloud(QAI, p) == 3){ // thin cloud (cirrus) = cloud
        qim_[p] = 2;
      } else if (get_cloud(QAI, p) == 1){ // buffered cloud = dilated cloud
        qim_[p] = 4;
      } else if (get_shadow(QAI, p)){ // shadow = shadow
        qim_[p] = 3;
      } else { // everything else = clear
        qim_[p] = 0;
      }
