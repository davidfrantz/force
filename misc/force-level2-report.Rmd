...
get_theme_colors <- function(theme = "auritus"){
  ...
  if (!file.exists(file)){
    available <- tools::file_path_sans_ext(list.files(path, pattern = "\\.js$"))
    stop("Unknown theme '", theme, "'. Available themes: ", paste(sort(available), collapse = ", "))
  }
  js_lines <- readLines(file)
  palette <- unlist(regmatches(js_lines, gregexpr("#[0-9a-fA-F]{6}", js_lines)))
}
colors <- get_theme_colors(params$theme)
```
...
if (nfail > 0) {
  pos <- which(data$status == "Error")
  cbind(flog[pos], data$log[pos])
}
```
...
*Processing status is not taken into account - unless an Error occured.*
...
The following plots count the number of processed Level 2 image chips per sensor, year, month and week. Only Successfully processed images are shown.
...
Only Successfully processed images are shown.
